...
这说明当前仓库虽然已经打通了大部分托管主链，但与原始仓库相比仍不是完整镜像；不过对这两个 native 模块，后续计划是通过 NuGet 获取已构建好的 DLL，而不是继续迁入源码。
...
- `WpfGfx` 源码未迁入，后续改为 NuGet 二进制接入
- `PenImc` 源码未迁入，后续改为 NuGet 二进制接入
...
1. 先确认 `PenImc`、`WpfGfx` 可用 NuGet 包来源
    - 原因：它们不再走源码迁移路线，需要先确认包的目标框架、平台和 DLL 列表。
2. 再把托管侧引用切换到二进制依赖
    - 原因：这样可以避开高维护成本 native 模块的环境依赖，并保持当前 C# 主链可继续推进。
...
- `System.Windows.Presentation` 已完成迁入，但这不意味着仓库已经等同于 origin；当前主要剩余偏离转为 `PenImc` / `WpfGfx` 仍未通过二进制包接入、打印动态边界、`PresentationUI` 标记编译占位，以及若干显式产物引用。
