...
MtExtern(CMilEmissiveMaterialDuce);

// Class: CMilEmissiveMaterialDuce
class CMilEmissiveMaterialDuce : public CMilMaterialDuce
{
    friend class CResourceFactory;

protected:

    DECLARE_METERHEAP_CLEAR(ProcessHeap, Mt(CMilEmissiveMaterialDuce));

    CMilEmissiveMaterialDuce(__in_ecount(1) CComposition* pComposition)
        : CMilMaterialDuce(pComposition)
    {
    }

    virtual ~CMilEmissiveMaterialDuce();
...
public:
    ...
    {
        return type == TYPE_EMISSIVEMATERIAL || CMilMaterialDuce::IsOfType(type);
    }
    ...
    CMilEmissiveMaterialDuce_Data m_data;
};
