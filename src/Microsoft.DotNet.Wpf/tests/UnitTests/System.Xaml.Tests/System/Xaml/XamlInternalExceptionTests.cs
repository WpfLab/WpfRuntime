...
public class XamlInternalExceptionTests
{
    [Fact]
    public void Ctor_Default()
    {
        var exception = new XamlInternalException();
    ...
    {
        var exception = new XamlInternalException("message");
        Assert.Equal("Internal XAML system error: message", exception.Message);
        Assert.Null(exception.InnerException);
        Assert.Equal(0, exception.LineNumber);
        Assert.Equal(0, exception.LinePosition);
    }
    ...
    {
        var exception = new XamlInternalException(message, innerException);
...
#pragma warning disable SYSLIB0011, SYSLIB0051 // Type or member is obsolete
    ...
    {
        Assert.Throws<ArgumentNullException>("info", () => new SubXamlInternalException(null!, new StreamingContext()));
    }
...
#pragma warning restore SYSLIB0011, SYSLIB0051 // Type or member is obsolete
    ...
    private class SubXamlInternalException : XamlInternalException
    {
        public SubXamlInternalException(SerializationInfo info, StreamingContext context) : base(info, context)
        {
        }
    }
}
